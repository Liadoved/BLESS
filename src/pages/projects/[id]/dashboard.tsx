  ...
  const fetchProjectData = async () => {
    try {
      const [projectRes, contactsRes, statsRes] = await Promise.all([
        ...
        fetch(`/api/projects/${id}/contacts`),
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-12">
          <h1 className="text-3xl font-bold text-gray-900 mb-2">
            {project.celebrantName} - ניהול פרויקט
          </h1>
          <p className="text-gray-600">
            {stats.daysUntilDeadline} ימים עד לתאריך היעד
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-12">
          <div className="bg-white rounded-lg shadow p-6">
            <div className="text-sm font-medium text-gray-500">סה"כ תמונות</div>
            <div className="mt-2 text-3xl font-semibold text-gray-900">{stats.totalImages}/70</div>
          </div>
          <div className="bg-white rounded-lg shadow p-6">
            <div className="text-sm font-medium text-gray-500">סה"כ סרטונים</div>
            <div className="mt-2 text-3xl font-semibold text-gray-900">{stats.totalVideos}</div>
          </div>
          <div className="bg-white rounded-lg shadow p-6">
            <div className="text-sm font-medium text-gray-500">ממתינים לתגובה</div>
            <div className="mt-2 text-3xl font-semibold text-gray-900">{stats.pendingContacts}</div>
          </div>
          <div className="bg-white rounded-lg shadow p-6">
            <div className="text-sm font-medium text-gray-500">השלימו</div>
        ...
        <div className="bg-white rounded-lg shadow">
          <div className="p-6 border-b border-gray-200">
            <div className="flex justify-between items-center">
              <h2 className="text-lg font-medium text-gray-900">אנשי קשר</h2>
                ...
                {contacts.map((contact) => (
                  <tr key={contact.id}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      >
                        {contact.status === 'uploaded' ? 'הושלם' :
                         contact.status === 'sent' ? 'נשלח' : 'ממתין'}
                      </span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
